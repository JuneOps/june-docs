...
- **Workspace Owner** or **Workspace Admin** access to create an Admin API key. A Workspace Admin can create a Custom key for non-sensitive permissions only; if a permission below isn't offered to your Admin account, a Workspace Owner needs to create the key.
...
<Steps>
  <Step title="Generate a workspace Admin API key">
    ...
    2. Go to [**Credentials → Admin keys**](https://admin.openai.com/credentials?tab=admin-keys) and select **Create new admin key**.
    3. Choose the workspace this key belongs to.
    4. Under **Custom permissions**, grant read access to **workspace users**, **Codex Enterprise analytics**, and **cost logs**, and **read and write** on **usage limits**. If your account doesn't offer one of these, a Workspace Owner needs to create the key instead.
    5. Copy the secret and store it somewhere safe. OpenAI shows it once; if you close the dialog first, you'll need to create a new key.
  </Step>
