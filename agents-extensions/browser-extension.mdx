...
<CardGroup cols={2}>
  ...
  <Card title="Configuration" icon="gear">
    Pulls your organization's list of tracked applications from June. This list refreshes automatically every 6 hours.
  </Card>
  ...
  <Card title="Reporting" icon="paper-plane">
    Sends usage data back to June throughout the day. Data older than 30 days is automatically discarded.
  </Card>
</CardGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Alarms" icon="clock">
    Keeps the extension running on schedule — refreshing configuration and sending usage reports.
  </Accordion>
...
The agent runs locally only — no network requests. It reads the device serial number and passes it to the extension for hardware-level identification.
...
<AccordionGroup>
  ...
  <Accordion title="How often does it send data?">
    Usage data is sent throughout the day as you work. It only reports on completed days, not the current one.
  </Accordion>
  ...
  <Accordion title="Does it affect browser performance?">
    No. The extension performs a lightweight check against your configured app list — no page content scanning or modification.
  </Accordion>
</AccordionGroup>
