...
<CardGroup cols={2}>
  ...
  <Card title="Reporting" icon="paper-plane">
    Sends collected usage data back to June as you work, with a background check every 6 hours to ensure nothing is missed. Data older than 30 days is automatically discarded from the device.
  </Card>
</CardGroup>
...
<AccordionGroup>
  ...
  <Accordion title="How often does it send data?">
    Usage data is sent automatically as you switch between applications, with a backup check every 6 hours. It only sends data for previous days to avoid conflicts with ongoing tracking.
  </Accordion>
