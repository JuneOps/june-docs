...
<CardGroup cols={2}>
  <Card title="MDM Integration" icon="link">
    Primary source: Devices managed by your MDM systems (Jamf Pro, Intune, Kandji)
  </Card>
  
  <Card title="Network Scanning" icon="wifi">
    Secondary discovery: Network-connected devices detected via scanning
  </Card>
  
  <Card title="Manual Addition" icon="plus">
    Direct entry: Manually add devices not discovered automatically
  </Card>
...
Each device page provides comprehensive information:
...
<Steps>
  <Step title="Access Device Settings">
    Navigate to Administration > Device Management > Custom Attributes
  </Step>
  ...
  <Step title="Configure Field Types">
    Choose appropriate field types:
    - Text fields for notes and IDs
    - Date fields for important dates
    - Number fields for costs and quantities
    - Dropdown lists for standardized values
  </Step>
  
  <Step title="Set Field Permissions">
    Control who can view and edit custom attributes
  </Step>
</Steps>
...
Configure network scanning for comprehensive discovery:

<Steps>
  <Step title="Define Network Ranges">
    Specify IP ranges to scan for devices
  </Step>
  
  <Step title="Set Discovery Schedules">
    Configure automated scanning frequency
  </Step>
  
  <Step title="Configure Detection Rules">
    Set rules for identifying and classifying discovered devices
  </Step>
</Steps>

### Integration-Based Discovery

Leverage existing systems for device discovery:

- **Active Directory**: Discover domain-joined devices
- **DHCP Servers**: Identify devices from DHCP leases
- **Network Monitoring Tools**: Import from existing monitoring systems
- **Asset Management Systems**: Sync with existing inventory databases
